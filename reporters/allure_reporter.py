...
class AllureReporter(BaseReporter):
    ...
    def _generate_categories_file(self):
        ...
        self.logger.debug(f"Allure 分类文件已生成: {categories_file}")
    ...
    def _generate_environment_file(self, environment_info: Dict[str, Any]):
        ...
        env_content = [f"{k}={v}" for k, v in environment_info.items()]
        env_file.write_text('\n'.join(env_content), encoding='utf-8')
        self.logger.debug(f"Allure 环境文件已生成: {env_file}")
    ...
    def generate(self, test_results: List[Any], **kwargs) -> str:
        ...
        """
        try:
            self.logger.info(f"开始生成 Allure 报告，共 {len(test_results)} 个测试用例")
            ...
            for i, test_result in enumerate(test_results):
                try:
                    ...
                    self.logger.debug(f"生成 Allure 结果文件: {result_file.name}")
                except Exception as e:
                    self.logger.error(f"生成第 {i+1} 个测试用例的 Allure 结果失败: {e}")
            ...
            self.logger.info(f"✅ Allure 结果文件已生成到: {self.output_dir}")

            # 提示用户如何生成HTML报告
            allure_report_dir = str(self.output_dir).replace("-results", "-report")
            self.logger.info(f"要生成 Allure HTML 报告，请运行以下命令：")
            self.logger.info(f"  allure generate {self.output_dir} -o {allure_report_dir} --clean")
            self.logger.info(f"  allure open {allure_report_dir}")

            return str(self.output_dir)

        except Exception as e:
            self.logger.error(f"生成 Allure 报告失败: {e}")
            return ""
    ...
    def generate_html_report(self, results_dir: Optional[str] = None,
        ...
        try:
            ...
            try:
                ...
                self.logger.error("Allure 命令行工具未安装")
                self.logger.error("请先安装 Allure: https://docs.qameta.io/allure/#_installing_a_commandline")
                return False

            # 生成报告
            cmd = ["allure", "generate", results_dir, "-o", report_dir, "--clean"]
            self.logger.info(f"正在生成 Allure HTML 报告...")
            result = subprocess.run(cmd, capture_output=True, text=True)

            if result.returncode == 0:
                self.logger.info(f"✅ Allure HTML 报告已生成: {report_dir}")
                return True
            else:
                self.logger.error(f"生成 Allure HTML 报告失败: {result.stderr}")
                return False

        except Exception as e:
            self.logger.error(f"生成 Allure HTML 报告异常: {e}")
            return False