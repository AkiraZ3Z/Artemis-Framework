...
class TemplateManager:
    ...
    def __init__(self, template_dirs: List[str] = None):
        ...
        logger.info(f"模板管理器初始化完成，搜索路径: {self.template_dirs}")
    ...
    def _init_jinja2_env(self):
        """初始化Jinja2环境"""
        if not HAS_JINJA2:
            logger.warning("Jinja2未安装，模板功能不可用")
            return
    ...
    def add_template_dir(self, template_dir: str, priority: int = 0):
        ...
        logger.info(f"添加模板目录: {template_dir} (优先级: {priority})")
    ...
    def copy_static_files(self, output_dir: str, config: Dict[str, Any]):
        ...
        if files_copied > 0:
            logger.debug(f"已复制 {files_copied} 个静态文件到: {target_static_dir}")
        else:
            logger.warning("未找到任何静态文件可复制")
...
class HTMLReporter(BaseReporter):
    ...
    def __init__(self, output_dir: str = "reports/html", config: Optional[Dict] = None):
        ...
        # 添加配置中的模板目录
        template_config = config.get("templates", {}) if config else {}
        ...
        logger.info(f"默认模板: {self.default_template}")
        logger.info(f"可用模板: {self.template_manager.get_available_templates()}")
    
    def _check_default_template(self):
        """检查默认模板是否存在"""
        template = self.template_manager.get_template(self.default_template)
        if not template:
            logger.warning(f"默认模板 '{self.default_template}' 未找到")
            logger.warning(f"可用模板: {self.template_manager.get_available_templates()}")
    ...
    def _create_builtin_templates(self):
        ...
        if not default_template_path.exists():
            ...
            logger.info(f"已创建默认模板: {default_template_path}")
...
.summary { background: #f5f5f5; padding: 20px; border-radius: 5px; }"""
        ...
        logger.info(f"已创建默认静态文件目录: {static_dir}")
    ...
    def _calculate_statistics(self, test_results: List[Any]) -> Dict[str, Any]:
        ...
        error_percent = (error / total * 100) if total > 0 else 0
    ...
    def _prepare_testcase_data(self, test_result: Any) -> Dict[str, Any]:
        ...
        if hasattr(test_result, 'step_results'):
            for step in test_result.step_results:
                step_data = {
                    ...
                    "status": getattr(step, 'status', 'unknown').value 
        ...
        return {
            ...
            "status": getattr(test_result, 'status', 'unknown').value 
                if hasattr(test_result.status, 'value') 
                else getattr(test_result, 'status', 'unknown'),
    ...
    def generate(self, test_results: List[Any], **kwargs) -> str:
        ...
        """
        if not HAS_JINJA2:
            logger.error("Jinja2未安装，无法生成HTML报告")
            logger.error("请运行: pip install Jinja2")
            return ""
        
        if not test_results:
            logger.warning("没有测试结果，跳过HTML报告生成")
            return ""
        ...
        try:
            ...
            if not template:
                ...
                logger.error(f"可用模板: {self.template_manager.get_available_templates()}")
                return ""
            ...
            # 使用配置中的文件名格式
            filename_config = self.config.get("filename", {}) if self.config else {}
            filename_format = filename_config.get("format", "test_report_{session_id}_{timestamp}.html")
